      ...
      const phaseScope = activePhase && activePhase.title
        ? [
          ...
          activePhase.number === 1
            ...
            : 'This phase\'s sub-tasks are NEW pages/files that DO NOT EXIST yet — you MUST CREATE them now with write_file. You may read ONE existing page first to match its design/header/footer/source-of-truth files, but reading is not the work: the deliverable is the new files. Later HTML pages must link the existing shared CSS and shared components/scripts; do NOT paste a new inline theme, nav, logo, footer, or button system into each page. Earlier phases built only part of the site, so do NOT conclude the project is "already complete" or offer a written plan instead of code — build this phase\'s pages.',
