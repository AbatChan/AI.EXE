      ...
      const phaseScope = activePhase && activePhase.title
        ? [
          ...
          activePhase.number === 1
            ...
            : 'Build only this phase\'s named deliverables. If a sub-task names a new file/page from Expected files, create it with write_file. If a sub-task is shared styling, components, behavior, README, or design/brand guidance, update that existing source-of-truth file instead of inventing a new public HTML page. You may read ONE existing page first to match its design/header/footer/source-of-truth files, but reading is not the work. Later HTML pages must link the existing shared CSS and shared components/scripts; do NOT paste a new inline theme, nav, logo, footer, or button system into each page. Never create standalone brand-strategy.html, visual-identity.html, typography.html, design-system.html, motion-system.html, SEO, CRO, or guide pages unless they are explicitly listed as public pages in Expected files.',
