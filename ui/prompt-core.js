  ...
  function createPromptCore(deps) {
    ...
    const promptTemplateDefaults = {
      ...
      developer_agent_plan: [
        ...
        'phases: empty string for a small/medium project. For a LARGE or multi-page project, 2-4 build phases separated by " | ". The phases must PARTITION the work — each expected_file/page/feature is built in EXACTLY ONE phase; NEVER repeat a page/feature across phases, and NEVER add a "polish/restructure/reorganize/finalize" phase that re-touches files earlier phases built (later phases only ADD new files/features). Phase 1 = a COMPLETE RUNNABLE minimal core (entry page + shared CSS/JS + nav/layout); each later phase ADDS a distinct NEW group. Title, then " :: ", then 2-5 CONCRETE file-grounded sub-tasks (name the actual pages/files) separated by " ; ". Together the phases cover every expected_file exactly once. Multi-page example: "Runnable core :: index.html hero+nav ; css/style.css ; js/script.js | Main pages :: about.html ; services.html ; portfolio.html | Detail pages :: pricing.html ; contact.html ; booking.html | Content pages :: blog.html ; blog-post.html ; faq.html". Single-app example (partition by feature): "Runnable skeleton :: board ; nav ; empty grid | Core gameplay :: movement ; line clears ; score | Extras :: animations ; pause/restart ; high-score". Phase 1 stands alone; later phases run on Continue.',
        'Rules:',
