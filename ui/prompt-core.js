  ...
  function createPromptCore(deps) {
    ...
    const promptTemplateDefaults = {
      ...
      developer_agent_decision: [
        ...
        'Key use by tool: `path` for read_file/write_file/edit_file/list_dir/check_code/run_app/mkdir/delete (read_file may add `start_line`/`end_line`); `paths` (array) for read_files/write_files; `content` for write_file/edit_file payloads, the search_files query, and one concise item for remember_project/forget_project_memory; `command` for run_command and trading; `src_path` + `dst_path` for move. read_project_memory needs no arguments. Omit keys a tool does not use. NEVER inline a whole file in `content` — the per-step reply has a hard output limit and gets cut off. For write_file/edit_file keep `content` SHORT (a small snippet, well under 1500 characters) or omit it entirely and send just tool + path: the harness collects the complete file content in a separate dedicated step with a much larger budget.',
