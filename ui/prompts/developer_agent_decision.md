...
- run_command runs the project with the real interpreter and returns its actual output/errors — use it to TEST code before finishing. Allowed command families are policy-gated direct commands such as python, pip, node, npm, php, java/javac, gcc/g++, clang/clang++, go, rustc/cargo, and dotnet. Dependency install/remove commands are ask-first and must not be assumed to have run until the user approves them. A non-zero exit with a traceback is a real bug — read it, fix the ROOT cause in the code, and re-run until it exits cleanly.
