...
phases: empty string for a small/medium project. For a LARGE or multi-page project, 2-4 build phases separated by " | ". The phases must PARTITION the work — each expected_file/page/feature is built in EXACTLY ONE phase; NEVER repeat a page or feature in two phases, and NEVER add a "polish/restructure/reorganize/finalize" phase that re-touches files earlier phases already built. Phase 1 = a COMPLETE RUNNABLE minimal core (entry page/main command + shared foundations + core structure/behavior); each later phase adds a distinct page/feature/doc deliverable from expected_files. Give each phase a short title, then " :: ", then 2-5 CONCRETE, file-grounded sub-tasks (name the actual pages/files/features) separated by " ; ". Phase order should be structure/core first, then feature groups, then extras — never style/decoration first. Together the phases must cover every expected_file exactly once. Multi-page example: "Runnable core :: index.html hero+nav ; css/design-tokens.css ; css/style.css ; js/components.js ; js/script.js | Product pages :: overview.html ; features.html ; workflow.html | Support pages :: help.html ; resources.html ; request.html | README :: README.md run+design notes". Single-app example (partition by feature, same idea): "Runnable skeleton :: index.html layout ; css/style.css ; js/app.js | Core workflow :: create records ; update status ; filter list | Extras :: saved views ; keyboard shortcuts ; import/export". Phase 1 stands alone; later phases run on Continue.
...
- BRAND/DESIGN/STRATEGY WORDS ARE NOT AUTOMATIC WEBSITE PAGES: terms like brand strategy, visual identity, typography, design system, motion system, CRO, SEO, content strategy, and implementation guide are usually guidance for HOW to build the site. Do NOT create /brand-strategy.html, /visual-identity.html, /typography.html, /design-system.html, /motion-system.html, /seo.html, /cro.html, or similar public pages unless the user explicitly asks for those as navigable site pages. Put reusable visual decisions in /css/design-tokens.css and /css/style.css, repeated markup in /js/components.js, behavior in /js/script.js, and written strategy/notes in /README.md only if docs are requested.
