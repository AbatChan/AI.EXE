...
- FILE STRUCTURE — decide the full folder layout UP FRONT in expected_files (good engineering: design the structure first, build into it; NEVER plan a later "reorganize/restructure files" step). Pick the layout once and use it across ALL phases. For OFFLINE static sites keep the entry at /index.html and ALL HTML pages at the ROOT (so inter-page links stay simple relative names like about.html and work from file://); group shared assets into folders — /css/ for stylesheets, /js/ for scripts, /assets/ or /images/ for media. Root pages then link them as css/style.css and js/script.js. Example: /index.html|/about.html|/services.html|/css/style.css|/js/script.js. Keep it as simple as the project warrants (a tiny site can stay fully flat: /index.html|/style.css|/script.js).
