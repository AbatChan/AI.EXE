...
phases: empty string for a small/medium project. For a LARGE or multi-page project, 2-4 build phases separated by " | ". The phases must PARTITION the work — each expected_file/page/feature is built in EXACTLY ONE phase; NEVER repeat a page or feature in two phases, and NEVER add a "polish/restructure/reorganize/finalize" phase that re-touches files earlier phases already built (later phases only ADD new files/features). Phase 1 = a COMPLETE RUNNABLE minimal core (the entry page + shared CSS/JS + nav/layout); each later phase ADDS a distinct NEW group built on the core. Give each phase a short title, then " :: ", then 2-5 CONCRETE, file-grounded sub-tasks (name the actual pages/files/features) separated by " ; ". Together the phases must cover every expected_file exactly once. Multi-page example: "Runnable core :: index.html hero+nav ; css/style.css ; js/script.js | Main pages :: about.html ; services.html ; portfolio.html | Detail pages :: pricing.html ; contact.html ; booking.html | Content pages :: blog.html ; blog-post.html ; faq.html". Single-app example (partition by feature, same idea): "Runnable skeleton :: board layout ; top nav ; empty grid renders | Core gameplay :: piece movement ; line clears ; live score | Extras :: animations ; pause/restart ; high-score persistence". Phase 1 stands alone; later phases run on Continue.
...
- MULTI-PAGE WEBSITES: when the user names several distinct PAGES (e.g. Home, About, Services, Portfolio, Contact), plan ONE HTML file per page (/index.html|/about.html|/services.html|/portfolio.html|/contact.html) PLUS a shared /style.css and /script.js — never collapse a multi-page site into a single /index.html. Building a several-page website is a real multi-file PROJECT even when the request also says "plan", "content structure", "design direction", "strategy", "SEO", or "implementation guide" — those describe the website to BUILD, not a single written document. Only plan a single document file when the user explicitly asks for ONLY a written plan/outline and no actual pages. For such multi-page builds, also fill `phases` (each phase = a coherent set of pages/features).
