...
phases: empty string for a small/medium project. For a LARGE or multi-page project, 2-4 build phases separated by " | ". The phases must PARTITION the work — each expected_file/page/feature is built in EXACTLY ONE phase; NEVER repeat a page or feature in two phases, and NEVER add a "polish/restructure/reorganize/finalize" phase that re-touches files earlier phases already built. Phase 1 = a COMPLETE RUNNABLE minimal core (shared foundations + entry page/main command + core behavior); for web, brand/design tokens and shared CSS/components come BEFORE index.html so the first page uses the real system. Each later phase adds a distinct page/feature/doc deliverable from expected_files. Give each phase a short title, then " :: ", then 2-5 CONCRETE, file-grounded sub-tasks (name the actual pages/files/features) separated by " ; ". Phase order should be source-of-truth foundations first, then entry/core, then feature groups, then docs/extras. Together the phases must cover every expected_file exactly once. Multi-page example: "Runnable core :: css/design-tokens.css brand tokens ; css/style.css base system ; js/components.js shared header/footer ; index.html hero+nav ; js/script.js interactions | Product pages :: overview.html ; features.html ; workflow.html | Support pages :: help.html ; resources.html ; request.html | README :: README.md run+design notes". Single-app example (partition by feature, same idea): "Runnable skeleton :: index.html layout ; css/style.css ; js/app.js | Core workflow :: create records ; update status ; filter list | Extras :: saved views ; keyboard shortcuts ; import/export". Phase 1 stands alone; later phases run on Continue.
