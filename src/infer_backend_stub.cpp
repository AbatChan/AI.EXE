...
bool ValidateGguf(const std::filesystem::path& model) {
  ...
  char magic[4] = {};
  in.read(magic, 4);
  return in.gcount() == 4 && magic[0] == 'G' && magic[1] == 'G' && magic[2] == 'U' && magic[3] == 'F';
}
...
std::string QuoteWindowsArg(const std::wstring& arg) {
  ...
  for (wchar_t c : arg) {
    ...
    char mb[4]{};
    const int wrote = WideCharToMultiByte(CP_UTF8, 0, &c, 1, mb, static_cast<int>(sizeof(mb)), nullptr, nullptr);
    if (wrote > 0) {
...
bool RunProcessCapture(const std::filesystem::path& executable,
  ...
  *exit_code = -1;
  ...
  for (;;) {
    DWORD bytes_read = 0;
    const BOOL ok = ReadFile(read_pipe, buffer, sizeof(buffer), &bytes_read, nullptr);
    if (!ok || bytes_read == 0) {
      break;
    }
...
std::filesystem::path ResolveEnginePath(const std::filesystem::path& self_path,
  ...
  auto check_candidate = [](const std::filesystem::path& candidate) -> std::filesystem::path {
    std::error_code ec;
  ...
  const auto base_dir = self_path.empty() ? std::filesystem::current_path() : self_path.parent_path();
#ifdef _WIN32
  const std::vector<std::string> names = {"llama-cli.exe", "llama_main.exe", "main.exe"};
#else
...
int main(int argc, char** argv) {
  ...
  if (self_test) {
    if (resolved_engine.empty()) {
      std::cerr << "missing local engine (expected llama-cli in runtime folder)";
      return 9;
    }
  ...
  if (resolved_engine.empty()) {
    std::cerr << "missing local engine: place llama-cli next to infer_backend executable "
                 "or set AI_EXE_LLM_ENGINE_PATH";
    return 13;
  }

  const std::string clean_prompt = CollapseWhitespace(prompt);
  ...
  int exit_code = -1;
  std::string run_err;
  if (!RunProcessCapture(resolved_engine, engine_args, &output, &exit_code, &run_err)) {
