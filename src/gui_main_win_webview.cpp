...
bool LaunchNodeDevServerWin(const std::filesystem::path &root, int port,
  ...
  bat << "@echo off\r\n"
      << "pushd \"" << root.string() << "\"\r\n"
      << "if errorlevel 1 (echo Could not open the project folder. & goto finish)\r\n"
      << "where npm >nul 2>nul && goto npmok\r\n"
      << "rem Node may be installed but off this process's PATH (installed after the app\r\n"
      << "rem launched, or via nvm/volta/per-user installers). Probe common locations and\r\n"
      << "rem prepend any that has npm. Flat lines (no paren block) so %ProgramFiles(x86)%\r\n"
      << "rem expanding to a path with ')' can't break batch's block parser.\r\n"
      << "if exist \"%ProgramFiles%\\nodejs\\npm.cmd\" set \"PATH=%ProgramFiles%\\nodejs;%PATH%\"\r\n"
      << "if exist \"%ProgramFiles(x86)%\\nodejs\\npm.cmd\" set \"PATH=%ProgramFiles(x86)%\\nodejs;%PATH%\"\r\n"
      << "if exist \"%LOCALAPPDATA%\\Programs\\nodejs\\npm.cmd\" set \"PATH=%LOCALAPPDATA%\\Programs\\nodejs;%PATH%\"\r\n"
      << "if exist \"%ProgramData%\\chocolatey\\bin\\npm.cmd\" set \"PATH=%ProgramData%\\chocolatey\\bin;%PATH%\"\r\n"
      << "if exist \"%LOCALAPPDATA%\\Volta\\bin\\npm.cmd\" set \"PATH=%LOCALAPPDATA%\\Volta\\bin;%PATH%\"\r\n"
      << "if exist \"%NVM_SYMLINK%\\npm.cmd\" set \"PATH=%NVM_SYMLINK%;%PATH%\"\r\n"
      << "where npm >nul 2>nul && goto npmok\r\n"
      << "echo Node.js is required to run this web project, and it was not found.\r\n"
      << "echo Install the LTS build from https://nodejs.org then click Run again.\r\n"
      << "echo (If you just installed it, this finds it without restarting the app.)\r\n"
      << "pause\r\n"
      << "exit /b 1\r\n"
      << ":npmok\r\n"
      << "rem A node_modules dir-only check misses interrupted installs - probe the runner binary.\r\n"
      << "set \"RUNNER=node_modules\\.bin\\"
      << (next_project ? "next.cmd" : "vite.cmd") << "\"\r\n"
      << "if not exist \"%RUNNER%\" (\r\n"
      ...
      << "  call npm install || call npm install --legacy-peer-deps || goto end\r\n"
      << ")\r\n"
      << "if not exist \"%RUNNER%\" (\r\n"
...
bool LaunchUpdater(const std::string &url, const std::string &version,
  ...
  ss << L"$ErrorActionPreference='SilentlyContinue'\r\n"
     ...
     // Graphite palette = ui/ai-exe.css dark tokens (panel/border2/text/text2/text3/accent/good/bad).
     << L"$acc=[Drawing.Color]::FromArgb(0,229,255)\r\n"
     << L"$bg=[Drawing.Color]::FromArgb(23,26,32)\r\n"
     << L"$line=[Drawing.Color]::FromArgb(48,53,63)\r\n"
     << L"$track=[Drawing.Color]::FromArgb(37,41,50)\r\n"
     << L"$text=[Drawing.Color]::FromArgb(232,235,240)\r\n"
     << L"$text2=[Drawing.Color]::FromArgb(163,169,180)\r\n"
     << L"$muted=[Drawing.Color]::FromArgb(130,136,150)\r\n"
     << L"$success=[Drawing.Color]::FromArgb(74,222,128)\r\n"
     << L"$fail=[Drawing.Color]::FromArgb(248,113,113)\r\n"
     ...
     << L"$script:phase='prep'; $script:pct=-1; $script:anim=0.0; $script:done=$false; $script:tc=0\r\n"
     ...
     // flash caused by the animated progress paints during an update.
     ...
     << L"$f.FormBorderStyle='None'; $f.Size=New-Object Drawing.Size(420,128)\r\n"
     << L"$f.StartPosition='CenterScreen'; $f.TopMost=$true; $f.BackColor=$bg; $f.Text='AI.EXE'\r\n"
     << L"$f.Region=New-Object Drawing.Region((RoundPath 0 0 $f.Width $f.Height 14))\r\n"
     << L"DoubleBuffer $f\r\n"
     << L"$f.Add_Paint({ param($s,$e) $g=$e.Graphics; $g.SmoothingMode='AntiAlias'; $pen=New-Object Drawing.Pen($line,1); $g.DrawPath($pen,(RoundPath 0 0 ($s.Width-1) ($s.Height-1) 14)); $pen.Dispose() })\r\n"
     ...
     << L"$timer.Add_Tick({\r\n"
     << L"  if($script:pct -lt 0){ $script:anim+=0.018; if($script:anim -ge 1){ $script:anim-=1 } }\r\n"
     << L"  $bar.Invalidate()\r\n"
     << L"  $script:tc++\r\n"
