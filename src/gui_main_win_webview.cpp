...
bool LaunchUpdater(const std::string &url, const std::string &version,
  ...
  ss << L"$ErrorActionPreference='SilentlyContinue'\r\n"
     ...
     << L"function RoundPath($w,$h,$r){ $p=New-Object Drawing.Drawing2D.GraphicsPath; $d=$r*2; $p.AddArc(0,0,$d,$d,180,90); $p.AddArc($w-$d-1,0,$d,$d,270,90); $p.AddArc($w-$d-1,$h-$d-1,$d,$d,0,90); $p.AddArc(0,$h-$d-1,$d,$d,90,90); $p.CloseAllFigures(); $p }\r\n"
     << L"$f.Region=New-Object Drawing.Region((RoundPath $f.Width $f.Height 18))\r\n"
     << L"$f.Add_Paint({ param($s,$e) $e.Graphics.SmoothingMode='AntiAlias'; $pen=New-Object Drawing.Pen($acc,2); $e.Graphics.DrawPath($pen,(RoundPath $s.Width $s.Height 18)); $pen.Dispose() })\r\n"
