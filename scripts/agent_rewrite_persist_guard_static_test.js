...
assert.match(runtime, /const persistPartials = !options \|\| options\.persistPartials !== false;/, 'generateFullAgentFile accepts a persistPartials switch');
...
assert.match(executor, /\{ persistPartials: creatingNewFile \}/, 'write path persists partials only for brand-new files');
