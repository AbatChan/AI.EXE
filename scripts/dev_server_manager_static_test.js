...
assert.match(aiExe, /function renderDevServerStatus\(rootRow\)/);
assert.match(aiExe, /devServerBelongsToWorkspace\(row\.cwd\)/);
assert.match(css, /\.msg-agent-devserver-btn/);
assert.match(css, /\.ws-root-server\.starting/);
...
assert.match(win, /Downloading the update/);
assert.doesNotMatch(win, /Downloading the update…/);
